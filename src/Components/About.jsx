...
const About = () => {
    ...
    return (
        <section id="about" className="py-20 px-4">
            <motion.div className='flex flex-col items-center justify-center w-full relative overflow-hidden' style={{ y: springY, scale: springScale }}>
                ...
                <div className='flex flex-col items-center justify-center w-full max-w-6xl'>
                    ...
                    <Transition>
                        <motion.div className='flex flex-col items-center justify-center w-full gap-12 mt-8 lg:gap-24' variants={containerVariants} initial="hidden" animate={isVisible ? "visible" : "hidden"}>
                            <motion.div className='flex flex-col items-center justify-center max-w-4xl' variants={itemVariants}>
                                <motion.div className='flex flex-col gap-y-4 text-center' initial={{ opacity: 0, x: -100, rotateY: -30 }} animate={{ opacity: 1, x: 0, rotateY: 0 }} transition={{ duration: 1, delay: 0.5, ease: [0.25, 0.46, 0.45, 0.94] }} >
                                    <motion.span className='text-md leading-relaxed md:text-lg' initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 2, delay: 0.8 }} whileInView={{ textShadow: ["0 0 0px rgba(0,0,0,0)", "0 0 2px rgba(0,0,0,0.1)", "0 0 0px rgba(0,0,0,0)"] }} viewport={{ once: false }} >
                                        I am creative and results-driven marketing professional with proven experience in content strategy, digital marketing, social media management, and copywriting. i am knnown for crafting compelling narratives that drive engagement and leading cross-functional teams to deliver high-impact campaigns.  i also demonstrates strong analytical thinking, storytelling skills, and a passion for brand building. i am adept at leveraging marketing tools to improve performance and reach.
                                    </motion.span>
                                </motion.div>
                                ...
                                <motion.div className='flex items-center justify-center mt-6' initial={{ opacity: 0, y: 50, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.8, delay: 1.2, type: "spring", stiffness: 200 }} >
                                    <motion.a download="Faidah's Resume" href="/faidah.pdf" className="flex custom-hover items-center bg-transparent text-lg gap-x-3 outlined px-10 md:px-16 font-medium !h-[45px] md:!h-[55px] transition-all duration-500 border-[2px] border-[#000] rounded-full hover:bg-black hover:text-white hover:border-transparent text-[#000] relative overflow-hidden group cursor-pointer" variants={buttonVariants} initial="idle" whileHover="hover" whileTap="tap">
                                        <motion.div className="absolute inset-0 bg-gradient-to-r from-black via-gray-800 to-black -translate-x-full group-hover:translate-x-0" transition={{ duration: 0.6, ease: "easeInOut" }} />

                                        <motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent -translate-x-full group-hover:translate-x-full" transition={{ duration: 0.8, delay: 0.2 }} />

                                        <motion.span className="relative z-10 flex items-center gap-2" whileHover={{ x: -2 }} transition={{ duration: 0.2 }}>
                                            <motion.svg className="button-icon relative z-10" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" variants={iconVariants} initial="idle" whileHover="hover">
                            ...
                            <motion.div className='w-full gap-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4' variants={containerVariants} initial="hidden" animate="visible" >
                                {skills.map((skill, index) => (
                                    <motion.div key={index} className='flex flex-col items-start justify-start gap-4 p-6 rounded-2xl hover:bg-gradient-to-br hover:from-gray-50 hover:to-white transition-all duration-500 cursor-pointer relative group border border-transparent hover:border-gray-100 hover:shadow-xl' variants={skillCardVariants} whileHover="hover" custom={index} style={{ transformStyle: "preserve-3d", }} >
                                        <motion.div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100" style={{ background: `linear-gradient(45deg, transparent, ${skill.color.split(' ')[0]?.replace('from-', '')}, ${skill.color.split(' ')[2]?.replace('to-', '')}, transparent)`, }} initial={{ rotate: 0 }} whileHover={{ rotate: 360 }} transition={{ duration: 2, ease: "linear" }} />
                                        ...
                                        <motion.div className="flex items-center gap-4 relative z-10">
                                            <motion.div className={`p-3 rounded-xl bg-gradient-to-br ${skill.color} shadow-lg`} animate={{ rotate: [0, 5, -5, 0], scale: [1, 1.05, 1] }} transition={{ duration: 4, repeat: Infinity, delay: index * 0.3, ease: "easeInOut" }} whileHover={{ scale: 1.1, rotate: 15, boxShadow: "0 10px 20px rgba(0,0,0,0.2)" }} >
                                                <motion.span className="text-white block" whileHover={{ rotate: 180 }} transition={{ duration: 0.4 }}>
                                                    {skill.icon}
                                                </motion.span>
                                            </motion.div>

                                            <motion.span className="font-bold text-lg lg:text-xl" whileHover={{ scale: 1.05, x: 5 }} transition={{ duration: 0.3 }}>
                                                {skill.title}
                                            </motion.span>
                                        </motion.div>

                                        <motion.span className="text-gray-600 leading-relaxed relative z-10" initial={{ opacity: 0.8 }} whileHover={{ color: "#374151" }} transition={{ duration: 0.3 }} >
                                            {skill.description}
                                        </motion.span>
                    ...
                    <motion.div className="fixed top-1/2 left-1/4 w-20 h-20 bg-gradient-to-br from-purple-400 to-pink-400 rounded-full pointer-events-none -z-10" style={{ filter: 'blur(2px)', opacity: 0.15 }} variants={pulseVariants} animate="animate" />
